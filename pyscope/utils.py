...
def get_subprocess_kwargs() -> Dict[str, Any]:
    """
    Get kwargs to hide console window on Windows EXE.
    Use with subprocess.run(..., **get_subprocess_kwargs())
    """
    kwargs = {}
...
def validate_pip_base_command(pip_cmd: List[str]) -> Tuple[bool, str]:
    """Validate that the pip command base is safe."""
    if not pip_cmd or not isinstance(pip_cmd, list):
        return False, "pip_command must be a non-empty list"
    
    executable = pip_cmd[0]
    if not isinstance(executable, str):
        return False, "Executable must be a string"
    
    # Validate executable and path
    if re.search(r'[;&|`$<>(){}\[\]*?]', executable):
         return False, "Executable path contains unsafe characters"

    exe_name = os.path.basename(executable).lower()
    
    if not any(term in exe_name for term in ['python', 'pip', 'conda', 'virtualenv']):
        return False, f"Executable '{exe_name}' not allowed. Must match: python, pip, conda, or virtualenv"
...
def run_pip_safe(args: List[str], timeout: int = 300, pip_cmd: Optional[List[str]] = None) -> str:
    ...
    try:
        ...
        if pip_cmd:
            ...
            valid, msg = validate_pip_base_command(pip_cmd)
            if not valid:
                raise ValueError(f"Invalid pip command: {msg}")
            ...
            cmd = [detector.get_actual_python_executable(), "-m", "pip"] + sanitized_args
...
def run_pip_with_real_progress(args: List[str], progress_callback=None, 
    ...
    # Wrap callback for signal support
    original_callback = progress_callback
    def progress_callback(data):
        if original_callback:
            original_callback(data)
        if signals and hasattr(signals, 'progress_updated'):
            signals.progress_updated.emit(data)
...
def get_environment_info(pip_cmd: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    try:
        # Determine Python path (use detector for frozen mode support)
        ...
        # Get pip version
        pip_version_cmd = pip_cmd or [detector.get_actual_python_executable(), "-m", "pip"]
        ...
        return {
            ...
            "pip_command": pip_cmd or [detector.get_actual_python_executable(), "-m", "pip"]
        }
    except Exception as e:
        ...
        return {
            ...
            "pip_command": [detector.get_actual_python_executable(), "-m", "pip"]
        }
...
def check_pip_available(pip_cmd: Optional[List[str]] = None) -> bool:
    ...
    try:
        cmd = pip_cmd or [detector.get_actual_python_executable(), "-m", "pip"]
        result = subprocess.run(
            cmd + ["--version"],
            capture_output=True,
            text=True,
...
# --- From version_comparator.py ---

class VersionComparator:
    """Compares versions using Python packaging standards."""
    
    def __init__(self):
        self._packaging_available = self._check_packaging()
    
    def _check_packaging(self) -> bool:
        """Check availability of packaging library."""
        try:
            import packaging.version
            return True
        except ImportError:
            logger.warning("packaging library not found, using fallback")
            return False
    
    def compare(self, version1: str, version2: str) -> Tuple[str, Optional[Any]]:
        """
        Compare two versions.
        
        Returns:
            ("older", info)   if version1 < version2
            ("newer", info)   if version1 > version2  
            ("same", info)    if version1 == version2
            ("error", error)  if an error occurs
        """
        if self._packaging_available:
            return self._compare_with_packaging(version1, version2)
        else:
            return self._compare_simple(version1, version2)
    
    def _compare_with_packaging(self, v1: str, v2: str) -> Tuple[str, Optional[Any]]:
        """Compare using packaging library (PEP 440)."""
        try:
            ...
            try:
                ...
                if ver1 < ver2:
                    return ("older", {"v1": str(ver1), "v2": str(ver2)})
                elif ver1 > ver2:
                    return ("newer", {"v1": str(ver1), "v2": str(ver2)})
                else:
                    return ("same", {"v1": str(ver1), "v2": str(ver2)})
                    
            except InvalidVersion as e:
                logger.warning(f"Invalid version format: '{v1}' or '{v2}' - {e}")
                # Fallback to simple comparison
                return self._compare_simple(v1, v2)
                
        except Exception as e:
            logger.error(f"Error in version comparison: {e}")
            return ("error", str(e))
    
    def _compare_simple(self, v1: str, v2: str) -> Tuple[str, Optional[Any]]:
        """Simple version comparison (fallback)."""
        try:
            # Parse into number lists
            ...
            for i in range(max_len):
                if v1_parts[i] < v2_parts[i]:
                    return ("older", {"v1_parts": v1_parts, "v2_parts": v2_parts})
                elif v1_parts[i] > v2_parts[i]:
                    return ("newer", {"v1_parts": v1_parts, "v2_parts": v2_parts})
            
            return ("same", {"v1_parts": v1_parts, "v2_parts": v2_parts})
    ...
    def _parse_version_parts(self, version: str) -> list:
        ...
        # Replace non-numeric with dot
        clean = re.sub(r'[a-zA-Z+-]', '.', str(version))
        parts = []
    ...
    def is_outdated(self, current: str, latest: str) -> bool:
        """Check if current version is older than latest (with normalization)."""
        if latest in ["Unknown", "Error", None, ""]:
            return False
        
        # Normalization
        current = str(current).strip().lower()
        latest = str(latest).strip().lower()
        
        if current == latest:
            return False
            
        try:
            result, _ = self.compare(current, latest)
            return result == "older"
        except:
            return False